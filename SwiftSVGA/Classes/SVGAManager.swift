...
open class SVGAManager: NSObject {
  ...
  public let processQueue = DispatchQueue(
    label: "com.swift.svga.manager.process", attributes: .concurrent)
  ...
  func loadLocal(url: URL, key: NSString, handle: CompletionHandler?) -> LoadTask? {
    ...
    self.processQueue.async {
      do {
        ...
        unionTask.finished(svga: svga, error: nil, url: url)
      } catch {
        unionTask.finished(svga: nil, error: error, url: url)
      }
    }
...
extension SVGAManager {
  public class UnionTaskCache {
    ...
    func set(_ unionTask: UnionTask?, key: NSString) {
      lock.lock()
      if unionTask != nil {
        unionTask?.onFinishedHandle = { [weak self] (_, cancel) in
  ...
  public class UnionTask {
    ...
    var onFinishedHandle: ((_ key: NSString, _ isCancel: Bool) -> Void)?
    ...
    func cancel(task: LoadTask) {
      lock.lock()
      callBackTasks.removeAll { loadTask in
        return task === loadTask
      }
      if callBackTasks.count == 0 {
        sessionTask?.cancel()
        self.isCancel = true
        self.onFinishedHandle?(self.key, true)
      }
      lock.unlock()
    }

    func finished(svga: SVGAMovieEntity?, error: Error?, url: URL) {
      lock.lock()
      let list = self.callBackTasks
      self.callBackTasks = []
      self.onFinishedHandle?(self.key, false)
      lock.unlock()
