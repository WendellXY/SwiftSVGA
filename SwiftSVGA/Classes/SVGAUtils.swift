...
extension Data {
  ...
  func zlibInflate() -> Data {
    ...
    var stream = z_stream()
    stream.next_in = withUnsafeBytes { rawBufferPointer in
      UnsafeMutablePointer(
        mutating: rawBufferPointer.baseAddress!.assumingMemoryBound(to: Bytef.self))
    }
    ...
    while !done {
      ...
      stream.next_out = decompressed.withUnsafeMutableBytes { rawBufferPointer in
        rawBufferPointer.baseAddress?.assumingMemoryBound(to: Bytef.self).advanced(
